...
const ParameterAnalyzer_1 = require("./src/analyzer/ParameterAnalyzer");
const ReturnValueAnalyzer_1 = require("./src/analyzer/ReturnValueAnalyzer");
const FunctionSummaries_1 = require("./src/analyzer/FunctionSummaries");
const results = [];
/**
 * Helper: Create a mock CFG for testing
 */
function createMockCFG(statements) {
 ...
 */
function test1_ParameterDirect() {
    console.log('\n[TEST 1] ParameterAnalyzer - Direct References');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result = analyzer.analyzeArgumentDerivation('x');
        const passed = result.type === ParameterAnalyzer_1.ArgumentDerivationType.DIRECT &&
            result.base === 'x' &&
 ...
 */
function test2_ParameterAddress() {
    console.log('\n[TEST 2] ParameterAnalyzer - Address-of Operations');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result = analyzer.analyzeArgumentDerivation('&x');
        const passed = result.type === ParameterAnalyzer_1.ArgumentDerivationType.ADDRESS &&
            result.base === 'x' &&
 ...
 */
function test3_ParameterExpression() {
    console.log('\n[TEST 3] ParameterAnalyzer - Expressions');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result = analyzer.analyzeArgumentDerivation('x + 1');
        const passed = result.type === ParameterAnalyzer_1.ArgumentDerivationType.EXPRESSION &&
            result.base === 'x' &&
            result.usedVariables.includes('x');
        return {
 ...
 */
function test4_ParameterComposite() {
    console.log('\n[TEST 4] ParameterAnalyzer - Composite Access');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result1 = analyzer.analyzeArgumentDerivation('obj.field');
        const result2 = analyzer.analyzeArgumentDerivation('ptr->member');
        const passed = result1.type === ParameterAnalyzer_1.ArgumentDerivationType.COMPOSITE &&
            result2.type === ParameterAnalyzer_1.ArgumentDerivationType.COMPOSITE &&
 ...
 */
function test5_ParameterCall() {
    console.log('\n[TEST 5] ParameterAnalyzer - Function Calls');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result = analyzer.analyzeArgumentDerivation('foo(x)');
        const passed = result.type === ParameterAnalyzer_1.ArgumentDerivationType.CALL &&
            result.base === 'foo' &&
            result.usedVariables.includes('x') &&
 ...
 */
function test6_ParameterArray() {
    console.log('\n[TEST 6] ParameterAnalyzer - Array Access');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const result = analyzer.analyzeArgumentDerivation('arr[i]');
        const passed = result.type === ParameterAnalyzer_1.ArgumentDerivationType.ARRAY_ACCESS &&
            result.base === 'arr' &&
 ...
 */
function test7_ParameterMapping() {
    console.log('\n[TEST 7] ParameterAnalyzer - Parameter Mapping');
    try {
        const analyzer = new ParameterAnalyzer_1.ParameterAnalyzer();
        const call = {
        ...
        const passed = mappings.length === 3 &&
            mappings[0].derivation.type === ParameterAnalyzer_1.ArgumentDerivationType.DIRECT &&
            mappings[1].derivation.type === ParameterAnalyzer_1.ArgumentDerivationType.EXPRESSION &&
            mappings[2].derivation.type === ParameterAnalyzer_1.ArgumentDerivationType.ADDRESS;
        return {
 ...
 */
function test8_ReturnVariable() {
    console.log('\n[TEST 8] ReturnValueAnalyzer - Variable Returns');
    try {
        const analyzer = new ReturnValueAnalyzer_1.ReturnValueAnalyzer();
        const cfg = createMockCFG(['int x = 5;', 'return x;']);
        const returns = analyzer.analyzeReturns(cfg);
        const passed = returns.length === 1 &&
            returns[0].type === ReturnValueAnalyzer_1.ReturnValueType.VARIABLE &&
            returns[0].value === 'x' &&
 ...
 */
function test9_ReturnExpression() {
    console.log('\n[TEST 9] ReturnValueAnalyzer - Expression Returns');
    try {
        const analyzer = new ReturnValueAnalyzer_1.ReturnValueAnalyzer();
        ...
        const returns = analyzer.analyzeReturns(cfg);
        const passed = returns.length === 1 &&
            returns[0].type === ReturnValueAnalyzer_1.ReturnValueType.EXPRESSION &&
            returns[0].usedVariables.includes('x');
        return {
...
function test10_ReturnCall() {
    ...
    try {
        ...
        const returns = analyzer.analyzeReturns(cfg);
        const passed = returns.length === 1 &&
            returns[0].type === ReturnValueAnalyzer_1.ReturnValueType.CALL &&
 ...
 */
function test11_ReturnConstant() {
    console.log('\n[TEST 11] ReturnValueAnalyzer - Constant Returns');
    try {
        const analyzer = new ReturnValueAnalyzer_1.ReturnValueAnalyzer();
        ...
        const returns = analyzer.analyzeReturns(cfg);
        const passed = returns.length === 1 &&
            returns[0].type === ReturnValueAnalyzer_1.ReturnValueType.CONSTANT &&
            returns[0].value === '42' &&
 ...
 */
function test12_ReturnMultiple() {
    console.log('\n[TEST 12] ReturnValueAnalyzer - Multiple Returns');
    try {
        const analyzer = new ReturnValueAnalyzer_1.ReturnValueAnalyzer();
 ...
 */
function test13_SummaryStrcpy() {
    console.log('\n[TEST 13] FunctionSummaries - strcpy');
    try {
        const summary = (0, FunctionSummaries_1.getFunctionSummary)('strcpy');
 ...
 */
function test14_SummaryMalloc() {
    console.log('\n[TEST 14] FunctionSummaries - malloc');
    try {
        const summary = (0, FunctionSummaries_1.getFunctionSummary)('malloc');
        const passed = summary !== null &&
            summary.name === 'malloc' &&
 ...
 */
function test15_SummaryCustom() {
    console.log('\n[TEST 15] FunctionSummaries - Custom Summaries');
    try {
        const summaries = new FunctionSummaries_1.FunctionSummaries();
...
// Run validation
runValidation();
//# sourceMappingURL=validate_phase4.js.map